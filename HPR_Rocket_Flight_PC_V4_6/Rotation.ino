...
void setCanards(){
  ...
  if(events.apogee){throwServo1 = throwServo2 = throwServo3 = throwServo4 = 20;}
...
float flight_angle = 86.95;  // from sensor
float width_AB = 2.0/12.0; // ft
float length_AB = 3.0/12.0; // ft
int num_AB = 4;
void setAirbrakes(){
  Serial.println("Setting Airbrakes!");
  float S; //area
  float Drag_now; // current drag
  float Cd_new; 
  float delta_S;
  float deltaAngle;
  float delta_Dreq;
  ...
  //Calculate the fin drag force
  //float Cd = 0.5; //estimate of the fin Cd
  float RocketArea =3.14159*(0.0508)*(0.0508) + 4*0.0508*0.0762*sinf(delta*angle_rad);//area of rocket including airbrakes in m^2
  float currentVel = fusionVel;
  unsigned long currentTime=micros();
  // Capture data from simFile to test airbreaks
  if(settings.testMode){
    //updateSimVars();
    currentTime=simVars.time;
    accelZ=simVars.accelZ;
    currentVel=simVars.fusionVel;
    offVert=simVars.offVert;
    airDensity = (simVars.baroPress * 100) / (R * (simVars.baroPress + 273.15));
  }
 
  if(fabs(currentVel)<1.0F){currentVel = 1.0F;}
  float dragForce = -(((accelZ*accel.gainZ)/(weight*sinf((offVert*.1)*angle_rad))) + weight); // accel.z*accel.gainZ in m/s^2*****
  int t=(currentTime/1000000UL + (0.5 / 2)) / 0.5 * 0.5;  // index for dragIdeal every 0.05 seconds

  delta_Dreq = dragIdeal[t] - dragForce;
  
  Cd = Cd+(2*delta_Dreq)/(airDensity*currentVel*currentVel*RocketArea); 
  delta_S = (2*delta_Dreq)/(airDensity*currentVel*currentVel*Cd); 
  if(delta_S < 0){
      int mult=-1;
  }else{
      int mult=1;
  ...
  deltaAngle = mult*asinf(abs(delta_S)/(num_AB*width_AB*length_AB)*angle_rad);
  
  if(deltaAngle > 4.0){ // servos can only move 4deg every .01 sec
    deltaAngle=4.0;
  }
  delta = delta + deltaAngle; // adjust for actual servo angle
  if(delta >= 80.0){
    delta = 80.0;
  } else if(delta < 0){
    delta=0;
  }
  //if we have hit apogee, then close the airbrakes
  //delta=45;
  if(events.apogee){delta=0;}
  delta = 0.6125*delta+56; // adjust for actual servo angle
  Serial.println("Deploying Airbrakes!"); Serial.print(delta);
  canardYaw1.write(delta-servo1trim);
  canardYaw2.write(delta-servo2trim);
  canardPitch3.write(delta-servo3trim);
  canardPitch4.write(delta-servo4trim);
...
void setRTB(){
  ...
  //if the mains have deployed, center the fins
  if(events.mainDeploy){throwServo1 = throwServo2 = 20;}
