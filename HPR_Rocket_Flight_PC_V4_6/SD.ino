...
void beginSD(){
  ...
  // Added for RADARS Testing:
  if(SD.exists("simFile.txt")){simFile=SD.open("simFile.txt", FILE_READ);}
  else{Serial.println("simFile does not exist!");}
}

void updateSimVars(){
  //Serial.println(micros());
  //Serial.println(simVars.time);
  String line = simFile.readStringUntil('\r');
  int varIndex=0;
  String currentVar="";
  for(int c=0; c<=line.length(); c++){ // look at each char of line
    if(line[c]==','){ // save variable when hits "","" then move on
      // edit struct values
      if(varIndex==0){simVars.time=currentVar.toFloat();}
      if(varIndex==3){simVars.accelZ=currentVar.toFloat();}
      if(varIndex==14){simVars.offVert=currentVar.toFloat();}
      if(varIndex==17){simVars.fusionVel=currentVar.toFloat();}
      if(varIndex==18){simVars.fusionAlt=currentVar.toFloat();}
      if(varIndex==19){simVars.fltEvents=currentVar;}
      if(varIndex==26){simVars.baroPress=currentVar.toFloat();}
      
      varIndex++; // next variable
      currentVar=""; // reset variable string
      continue;
    }
    currentVar+=line[c]; // append char to variable string
  }
